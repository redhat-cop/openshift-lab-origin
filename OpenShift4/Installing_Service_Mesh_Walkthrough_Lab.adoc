...
[NOTE]
The installation instructions https://docs.openshift.com/container-platform/4.1/service_mesh/service_mesh_install/installing-ossm.html
...
+
. Create the custom resource file `/rooot/cr-ocp.yaml` to create the Service Mesh.
+
Note the following:
...
* You are specifying a specific tag for the Jaeger pods (0.13.1). This is because otherwise the pods would be configured for 0.12 - and those tags don't exist in the Red Hat registry.
+
[source,texinfo]
----
cat << EOF >$HOME/cr-ocp.yaml
...
[source,sh]
----
oc create -f $HOME/cr_ocp.yaml -n istio-system
----
