...
* Using `buildah` task clone the source code from Gogs, build the container image and push it to the OpenShift registry
...
You tested the generic pipeline by adding pipelineruns using these pipelineruns to execute your generic pipeline twice - for two different applications from two different repositories
