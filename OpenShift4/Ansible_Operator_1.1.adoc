...
- path: ./patches/csv.yaml
  target:
    ...
    kind: ClusterServiceVersion
    name: gitea-operator.v0.0.0
    namespace: placeholder" >> ./config/manifests/kustomization.yaml
----
...
. Push the bundle image to the Quay registry and then validate that it looks correct (on Linux use `podman` instead of `docker`).
+
[source,sh]
----
docker push quay.io/$QUAY_ID/gitea-operator-bundle:v$BUNDLE_VERSION
operator-sdk bundle validate quay.io/$QUAY_ID/gitea-operator-bundle:v$BUNDLE_VERSION --image-builder docker
----
